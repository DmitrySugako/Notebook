...
public class FileNotesDAO implements NotesDAO {
	...
	public List<Note> load() {
		...
		try {
			try (BufferedReader br = new BufferedReader(new FileReader("D:\\Workspaces\\Notebook\\src\\resources\\note.txt"))) {
				String line;
				while ((line=br.readLine()) != null) {
					tempNotes.add(line);
				}
				br.close();
				System.out.println(tempNotes);
				System.out.println(tempNotes.get(1));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
